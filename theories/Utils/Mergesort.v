From Stdlib Require Import List Permutation Sorted Orders.

Import ListNotations.

Class TotalLeBool {A} (leb : A -> A -> bool) :=
  { leb_total : forall x y, is_true (leb x y) \/ is_true (leb y x) }.

Section compare.

    Context {A : Type}.
    Context (leb : A -> A -> bool).
  ...
  Context `{HLeb : TotalLeBool A leb}.
  ...
  Proof.
    intro; eapply Sorted_LocallySorted_iff, LocallySorted_sort; auto.
  Qed.

  Theorem Permuted_sort : forall l, Permutation l (sort l).
  Proof.
    intros; apply (Permuted_iter_merge l []).
  Qed.

  Corollary StronglySorted_sort : forall l,
    Transitive leb -> StronglySorted leb (sort l).
  Proof.
    auto using Sorted_StronglySorted, Sorted_sort.
  Qed.
